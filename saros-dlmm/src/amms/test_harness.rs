...
pub const SAROS_MINT: Pubkey =
    Pubkey::from_str_const("SarosY6Vscao718M4A778z4CGtvcwcGef5M9MEH1LGL");
...
pub const MASHA_MINT: Pubkey =
    Pubkey::from_str_const("mae8vJGf8Wju8Ron1oDTQVaTGGBpcpWDwoRQJALMMf2");
pub const C98_MINT: Pubkey = Pubkey::from_str_const("C98A4nkJXhpVZNAZdHUA95RpTF3T4whtQubL3YobiUX9");

lazy_static! {
    // For SwapMode::ExactIn
    pub static ref TOKEN_MINT_AND_IN_AMOUNT: [(Pubkey, u64); 6] = [
        (Pubkey::from_str_const(&spl_token::native_mint::ID.to_string()), 2_500_000_000),
        (SAROS_MINT, 1_000_000_000),
        (MASHA_MINT, 1_000_000_000),
        (USDC_MINT, 1_110_000_000),
        (USDT_MINT, 1_110_000_000),
        (C98_MINT, 1_000_000_000),
    ];

    // For SwapMode::ExactOut
    pub static ref TOKEN_MINT_AND_OUT_AMOUNT: [(Pubkey, u64); 6] = [
        (Pubkey::from_str_const(&spl_token::native_mint::ID.to_string()), 100_000_000),
        (SAROS_MINT, 500_000_000),
        (MASHA_MINT, 500_000_000),
        (USDC_MINT, 50_000_000),
        (USDT_MINT, 50_000_000),
        (C98_MINT, 500_000_000),
    ];
...
impl AmmTestHarnessProgramTest {
    ...
    pub async fn assert_out_amount_matches_simulated_swap(
        ...
        let process_transaction_result = self
            .process_transaction(&[swap_instruction], &[&user_keypair])
            .await;
    ...
    pub async fn assert_quote_matches_simulated_swap(
        ...
        let quote = if let Some(expected_error) = expected_error {
            ...
            match expected_error.downcast_ref::<anyhow::Error>() {
                Some(error) => {
                    let quote_error = quote_err.downcast_ref::<anyhow::Error>().unwrap();
                    // assert_eq!(error, quote_error);
                    assert_eq!(error.to_string(), quote_error.to_string());
                }
                None => {
    ...
    async fn send(
        ...
        match request {
            RpcRequest::GetAccountInfo => {
                let pubkey = Pubkey::from_str_const(params[0].as_str().unwrap());
...
impl AmmTestHarness {
    ...
    pub async fn load_program_test(
        ...
        pt.deactivate_feature(Pubkey::from_str_const(
...
async fn setup_token_accounts(
    ...
    let mut setup_ixs = vec![system_instruction::transfer(
        &Pubkey::from_str_const(&context.payer.pubkey().to_string()),
        &Pubkey::from_str_const(&wallet.to_string()),
        1_000_000_000,
    )];
